...
| `Spells.xml` Hunt's End wiring | Shiradi Champion's T5 selector "Beguile or Hunt's End" emits a `<SpellLikeAbility><Item>Hunt's End</Item></SpellLikeAbility>` (added by `patchEpicStrikes.mjs`), but no matching `<Spell>` entry exists upstream — the SLA pipeline drops it because `spellByName.get("Hunt's End")` is `undefined`, so the ability never reaches the Ranged rotation palette. | Added a `<Spell><Name>Hunt's End</Name>` entry modeled as a 1-hit ranged `<WeaponAttack>`: `<Scalar>3.5</Scalar>` (+250% damage) and `<CritRangeBonus>20</CritRangeBonus>`, matching the enhancement description verbatim. Marked `<PlaceholderDamage/>` since it's pure weapon damage with no dice rolls. | DDOBuilderV2 doesn't surface SLA damage at all — they only render the enhancement entry — so the catalog never had a Hunt's End spell. Mirrors the pattern used for Pluck of a String / Strike a Chord etc. |
| `src/engine/dps/abilities.ts::inferAttackMode` ranged-weapon variants | Description prose for Inquisitive `Shoot First` reads "Shoot First: Bow or Crossbow Attack: …", not the canonical "Ranged Attack:". The base regex (`/\branged attack:/i`) didn't match, so the ability ended up tagged `attackMode='magic'` and the Ranged editor's filter dropped it. | Extended `inferAttackMode` with a second pattern: `/\b(?:bow\|crossbow\|thrown\|shuriken)(?:\s+(?:or\|and\|&\|,)\s+…)?\s+attack:/i`. Catches "Bow Attack:", "Crossbow Attack:", "Bow or Crossbow Attack:", "Bow and Crossbow Attack:", etc. without false-positive risk on the no-colon descriptive phrasing. | Code-side fix, no XML rewrite — Inquisitive isn't the only tree that uses weapon-keyword attack markers, so a regex extension is more durable than re-prose'ing the upstream description. |
| `src/engine/dps/abilities.ts::collectClickieAbilities` action/reaper-boost category override | Inquisitive `No Holds Barred` has `<Category>cc</Category>` in upstream Inquisitive.tree.xml even though it's an Action Boost self-buff (`<UsesActionBoostCharge/>`, "Action Boost: …reload itself for 18 seconds"). With category 'cc' the ability lands in the CC tab and gets `attackMode='magic'`, so it never appears in the Ranged editor's Boost row. | After parsing `item.category`, override to `category='boost'` (and `placeholderDamage=false`) whenever `item.usesActionBoostCharge` or `item.usesReaperCharge` is true. Action-boost / reaper-boost charge users are buffs by definition. | The XML's `<Category>cc</Category>` on No Holds Barred is an upstream tagging bug — every other entry with `<UsesActionBoostCharge/>` (Deepwood Stalker, Battle Engineer, etc.) carries `<Category>boost</Category>`. A code-side override means we patch No Holds Barred without forking the upstream tree XML, and any future similarly mistagged action boost stays correct. |
| `Fatesinger.tree.xml`, `Inquisitive.tree.xml`, `Artificer_BattleEngineer.tree.xml`, `Cleric_Warpriest.tree.xml`, `FavoredSoul_WarSoul.tree.xml`, `Spells.xml` Ranged clickie damage | Selector-option clickies for several ranged-attack abilities (Arrow of Discord, Caustic Shot, Shatter Defenses Crossbow, Shoot First, Smite Foe Ranged, The Pluck of a String) carried `<PlaceholderDamage/>` but no `<WeaponAttack>` block, so the Ranged editor's damage map produced zero DPC for each — the abilities showed up in the rotation palette but contributed nothing. Their melee twins had the same problem. | Added inline `<WeaponAttack><Hits>1</Hits><Scalar>X</Scalar>[<CritRangeBonus>N</CritRangeBonus>]</WeaponAttack>` blocks per the in-game description prose: Arrow of Discord 1.2 (+2 crit range), Blade of Discord 1.3 (+2), Caustic Shot/Strike 1.3 (rank-3 max; +3d6 acid + 6d6 rust elemental damage unmodeled), Shatter Defenses Crossbow/Melee 1.5 (rank-3 max), Shoot First 1.4 (Shot Wound DoT unmodeled), Smite Foe Melee/Ranged 1.2, The Pluck of a String 1.3 with +4 crit range (the 2d10/CL Sonic spell damage remains modeled via `<SpellDamage>`), The Sword Sings 1.5 +4 crit range. | DDOBuilderV2 doesn't model SLA / clickie damage at all; they just render the description prose. Our ranged DPS pipeline reads `WeaponAttack.scalar` to compute per-cast damage, so we have to add the scalars somewhere. Inlining them on the source XML keeps the patches localized and visible to anyone editing the tree files. |
